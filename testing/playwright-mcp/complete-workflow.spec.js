...
test.describe('Complete ResearchHub Workflow', () => {
  test('Full workflow: Create study with blocks, publish, apply, complete', async ({ browser }) => {
    ...
    try {
      ...
      // Try to find login link/button - could be "Login" or "Sign in"
      const loginSelectors = ['text=Login', 'text=Sign in', 'a[href*="login"]', 'button:has-text("Login")', 'button:has-text("Sign in")'];
      ...
      await researcherPage.waitForLoadState('networkidle');
      await researcherPage.waitForSelector('input[type="email"], input[name="email"]');
      await researcherPage.fill('input[type="email"], input[name="email"]', RESEARCHER_EMAIL);
      await researcherPage.fill('input[type="password"], input[name="password"]', PASSWORD);
      
      // Try multiple submit button selectors
      const submitSelectors = ['button[type="submit"]', 'input[type="submit"]', 'button:has-text("Sign in")', 'button:has-text("Login")', '.btn-primary'];
      ...
      try {
        await researcherPage.waitForSelector('[data-testid="dashboard"], .dashboard, h1:has-text("Dashboard"), .main-dashboard', { timeout: 10000 });
        console.log('✅ Researcher login successful');
      } catch (e) {
        console.log('⚠️ Dashboard not found, continuing...');
        await researcherPage.screenshot({ path: 'test-results/login-state.png' });
      }

      // Navigate to Studies page - try multiple approaches
      console.log('🔍 Navigating to Studies page...');
      
      // Method 1: Try direct navigation to studies page
      await researcherPage.goto(`${BASE_URL}/app/studies`);
      await researcherPage.waitForLoadState('networkidle');
      
      // Check if we're on the studies page
      let studiesPageFound = false;
      try {
        await researcherPage.waitForSelector('[data-testid="studies-page"], .studies-page, h1:has-text("Studies"), .studies-container', { timeout: 3000 });
        studiesPageFound = true;
        console.log('✅ Studies page accessed via direct navigation');
      } catch (e) {
        console.log('⚠️ Direct navigation failed, trying navigation menu...');
      }
      ...
      if (!studiesPageFound) {
        ...
        const studiesSelectors = [
          ...
          'a[href*="studies"]',
          'a[href="/app/studies"]',
          '[data-testid="studies-link"]',
          'nav a:has-text("Studies")',
          '.nav-link:has-text("Studies")'
        ];
        ...
        for (const selector of studiesSelectors) {
          try {
            ...
            if (await element.isVisible()) {
              ...
              await researcherPage.waitForLoadState('networkidle');
              studiesPageFound = true;
              console.log(`✅ Studies page accessed via ${selector}`);
              break;
            }
          } catch (e) {
            console.log(`Studies navigation failed: ${selector}`);
          }
        }
      }
