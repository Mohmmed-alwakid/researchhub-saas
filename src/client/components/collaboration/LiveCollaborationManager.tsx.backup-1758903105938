...
interface WebSocketMessage {
  ...
  data?: {
    ...
    elementId?: string;
    action?: 'editing' | 'viewing' | 'commenting' | 'stopped' | string;
    field?: string;
    content?: string;
    type?: string;
    entityType?: string;
...
const LiveCollaborationManager: React.FC<LiveCollaborationManagerProps> = ({
  ...
  const handleActivityUpdate = useCallback((message: WebSocketMessage) => {
    if (message.data) {
      // Map incoming type to valid CollaborationActivity type
      const activityType = (message.data.type as string);
      const validType: CollaborationActivity['type'] = 
        ['user_joined', 'user_left', 'block_added', 'block_edited', 'block_removed', 
         'comment_added', 'study_published', 'participant_invited', 'approval_requested',
         'template_applied', 'settings_changed'].includes(activityType) 
          ? activityType as CollaborationActivity['type']
          : 'settings_changed';

      const category = (message.data.category as string);
      const validCategory: CollaborationActivity['category'] = 
        ['content', 'collaboration', 'administration', 'system'].includes(category)
          ? category as CollaborationActivity['category']
          : 'content';
  ...
  const startEditing = useCallback((elementType: 'block' | 'setting' | 'participant' | 'template', elementId: string, field?: string) => {
    sendMessage({
      type: 'edit_operation',
      roomId: `study:${studyId}`,
      data: {
  ...
  const stopEditing = useCallback((elementType: 'block' | 'setting' | 'participant' | 'template', elementId: string) => {
    sendMessage({
      type: 'edit_operation',
      roomId: `study:${studyId}`,
      data: {
  ...
  // Create collaboration context for child components
  ...
  // Make context available to children via data attributes for easy access
  const contextData = JSON.stringify(collaborationContext);

  return (
    <div className="live-collaboration-manager" data-collaboration-context={contextData}>
