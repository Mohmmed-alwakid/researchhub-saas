...
const StudySessionPage: React.FC = () => {
  ...
  const [useBlockSystem, setUseBlockSystem] = useState(true); // Enable block system by default
  ...
  useEffect(() => {
    const fetchStudy = async () => {
      try {
        ...
        // Get token from auth storage (same as api.service.ts)
        let token = null;
        ...
        // First, create or get the study session
        ...
        // Now get the full session with study data
  ...
  const startSession = async () => {
    try {
      ...
      // Create a new study session using the study-sessions API
  ...
  const completeSession = async () => {
    ...
    try {
      const response = await fetch(`/api/study-sessions/${session.id}?action=complete`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        ...
        body: JSON.stringify({
          finalData: { completedTasks: currentTaskIndex + 1 },
          completionNotes: 'Study completed successfully'
        })
      });
