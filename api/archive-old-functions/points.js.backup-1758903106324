...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'admin-balances' && req.method === 'GET') {
      ...
      // Transform the data to match frontend expectations (profile -> user)
      ...
      return res.status(200).json({
        success: true,
        data: { balances: transformedBalances }
      });
    }
    ...
    if (action === 'admin-transactions' && req.method === 'GET') {
      ...
      const { data: transactions, error } = await supabase
        ...
        .select(`
          *,
          profile:profiles!points_transactions_user_id_fkey(email, first_name, last_name, role),
          assigned_by_profile:profiles!points_transactions_assigned_by_fkey(email, first_name, last_name)
        `)
        .order('created_at', { ascending: false })
