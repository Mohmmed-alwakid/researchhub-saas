...
$loginResult = Test-APICall -Url "$apiBase/auth/login" -Method POST -Body ($researcherCredentials | ConvertTo-Json) -Description "Researcher Login"

if ($loginResult.success) {
    $testState.researcherToken = $loginResult.data.accessToken
    Write-Host "  👤 Logged in as: $($loginResult.data.user.firstName) $($loginResult.data.user.lastName) ($($loginResult.data.user.role))"
    
    # Create study
    $headers = @{ Authorization = "Bearer $($testState.researcherToken)" }
    $studyResult = Test-APICall -Url "$apiBase/studies" -Method POST -Body $studyData -Headers $headers -Description "Study Creation"
    
    if ($studyResult.success) {
        $testState.studyId = $studyResult.data._id
        Write-Host "  📊 Study created with ID: $($testState.studyId)"
        Write-Host "  📝 Study title: $($studyResult.data.title)"
        Write-Host "  🎯 Target participants: $($studyResult.data.targetParticipants)"
        Write-Host "  💰 Compensation: `$$($studyResult.data.compensation)"
        
        # Publish study
        $publishResult = Test-APICall -Url "$apiBase/studies/$($testState.studyId)/publish" -Method POST -Headers $headers -Description "Study Publishing"
        
        if ($publishResult.success) {
            Write-Host "  🚀 Study published successfully"
        }
    }
} else {
    $testState.errors += "Phase 1 Failed: $($loginResult.error)"
}

# Phase 2: Participant Login and Application
Write-Host "`n👤 Phase 2: Participant Login and Application" -ForegroundColor Cyan
Write-Host "-" * 50

$participantLoginResult = Test-APICall -Url "$apiBase/auth/login" -Method POST -Body ($participantCredentials | ConvertTo-Json) -Description "Participant Login"
